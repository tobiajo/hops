...
public class Utils {
    ...
    public static void addFileToLocalResources(Map<String, LocalResource> localResources, Configuration yarnConf, ApplicationId appId, String fileSrcPath, String fileDstPath) throws IOException {
        FileSystem fs = FileSystem.get(yarnConf);
        Path dst = new Path(fs.getHomeDirectory(), buildPath(".yarntf", appId.toString(), fileDstPath));
        ...
        localResources.put(fileDstPath, LocalResource.newInstance(url, type, visibility, size, timestamp));
    }

    public static void addClasspathToEnvironment(Map<String, String> environment, Configuration yarnConf) {
